...
// General rate limiter — 60 requests per minute per IP (across all /api routes)
...
// Strict limiter for payment code creation — max 5 per minute per IP
const paymentCodeLimiter = rateLimit({
  windowMs     : 60_000,
  max          : 5,
  standardHeaders: true,
  legacyHeaders: false,
  message      : { error: "Too many payment code requests. Please wait a minute and try again." },
  keyGenerator : (req) => req.ip + ':payment-codes',
});

app.use("/api/", generalLimiter);
app.use("/api/payment-codes", paymentCodeLimiter);
